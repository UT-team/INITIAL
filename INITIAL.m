...
(* ::Input::Initialization:: *)
psiCalc::usage="\
psiCalc[AMatrix] calculates the psi-matrix.";
...
(* ::Input::Initialization:: *)
checkDegrees::usage="\
checkDegrees[psi] computes the degrees in epsilon of the first row of Inverse[psi]. Output is the following list: {{dmin},{dmax,b1,...,bn}}, where dmin is the minimum power of epsilon of the GCD, dmax is the maximal degree of the GCD and b1 to bn are the maximal degrees of the bs.";


(* ::Input::Initialization:: *)
eps0Calc::usage="\
eps0Calc[psi] computes the eps^0 part of the bs (not normalized)."


(* ::Input::Initialization:: *)
equStep::usage="\
equStep[psi,phi,epsOrder,TVariables,degrees] computes the equation at order \"epsOrder\". \"TVariables\" are the Dot products used to describe the list \"phi\", and \"degrees\" is the output of checkDegrees.";
...
(* ::Input::Initialization:: *)
basisChange::usage="\
basisChange[AMatrix,TMatrix] uses FiniteFlow to compute the basis change given through \"TMatrix\" of the differential equation given through \"AMatrix\"."
...
psiStep[startA_,stepA_,OptionsPattern[]]:=Module[{opt,sz,graphvars,xv,outA},
...
FFpsiStep[startAIn_,stepAIn_,OptionsPattern[]]:=Module[{outA,pA,qA,psiGraph,input,starANode,stepANode,derNode1,derNode2,graphvars,mulNode,addNode,sz,xv,startA,stepA,
...
Do[
If[FFQ,
Aco[i]=FFpsiStep[Am,Aco[i-1],Sequence@@FilterRules[opt,Options[FFpsiStep]]],
Aco[i]=psiStep[Am,Aco[i-1],Sequence@@FilterRules[opt,Options[psiStep]]];
];
...
phiStep[startB_,stepBIn_,varsIn_,sol1_,OptionsPattern[]]:=Module[{opt,graphvars,xv,stepBmul,stepBder,varsTd,lsz,stepout,vars,oldPhi,stepBmulM,zpos},
...
FFphiStep[startB_,stepBIn_,varsIn_,sol1_,OptionsPattern[]]:=Module[{xv,graphvars,pB,qB,phiGraph,input,derNode1,derNode2,mul,solRedLs,solRedLsVars,
...
Do[
If[FFQ,
{{oldPhi,oldVars},{Bco[i],varsTd[i]}}=FFphiStep[startB,{phi1[[i-1,epsor]],Bco[i-1]},{vars[[i-1,epsor]],varsTd[i-1]},sol1,
Sequence@@FilterRules[Join[opt,{"Last"->(i===(sz+1))}],Options[FFphiStep]]];
,
{{oldPhi,oldVars},{Bco[i],varsTd[i]}}=phiStep[startB,{phi1[[i-1,epsor]],Bco[i-1]},{vars[[i-1,epsor]],varsTd[i-1]},sol1,
...
(* ::Input::Initialization:: *)
Options[phiStepOld]:=Options[psiStep];
phiStepOld[startB_,stepB_,varsIn_,sol1In_,epsor_,OptionsPattern[]]:=Module[{xv,graphvars,pB,qB,sol1,phiGraph,input,stepBNode,derNode1,derNode2,mul,solRedLs,solRedLsVars,
addpattrn,inputnodes,takepattern,varsTd,deletepos,
nummul,tp,stepout,num,take,muloutNode,stepBoutNode,lsz,opt,nzlearn,nznode},
opt = (#[[1]]->OptionValue[#[[1]]])&/@Options[phiStepOld];
...
{pB,qB}=Transpose[NumeratorDenominator[stepB]];
sol1=Join[sol1In,{T[a___]:>0/;(Length[{a}]>(epsor+1))}];
varsTd[0]=varsIn;
lsz=Length[startB];
...
(*build input nodes*)
Do[FFAlgRatFunEval[phiGraph,m[i],{input},graphvars,{startB[[i]]}],{i,Length[startB]}];
FFAlgRatFunEval[phiGraph,stepBNode,{input},graphvars,stepB];
...
If[varsTd[0]=!={},
(*Do the multiplication with auxiliary nodes*)
Do[FFAlgMatMul[phiGraph,mul[l],{m[l],stepBNode},1,1,Length[varsTd[1-1]]],{l,lsz}];
varsTd[1]=Table[Append[#,m[l]]&/@varsTd[0],{l,lsz}];
(*Check the reduction of the Td variables*)
solRedLs=varsTd[1]//.sol1//Expand;
solRedLs=Map[If[Head[#]===Plus,List@@#,{#}]&,solRedLs,{2}];
...
FFGraphOutput[phiGraph,stepBoutNode];
FFAlgNonZeroes[phiGraph,nznode,{stepBoutNode}];
FFGraphOutput[phiGraph,nznode];
nzlearn=FFNonZeroesLearn[phiGraph];
varsTd[1]=varsTd[1][["NonZero"/.nzlearn]];
...
(* ::Input::Initialization:: *)
Options[phiCalcOld]:=Options[psiCalc];
phiCalcOld[letters_,sol1_,epsor_,sz_,OptionsPattern[]]:=Module[{phi1,varsTd,time,stepB,startB,Bco,graphvars,allVars,xv,eps,i,opt,print},
opt = (#[[1]]->OptionValue[#[[1]]])&/@Options[phiCalcOld];
...
varsTd[0]={T[v0]};
stepB={1};
(*varsTd[1]=Table[T[v0,m[i]],{i,Length[letters]}];*)
...
Monitor[
Do[
{Bco[i],varsTd[i]}=phiStep[startB,Bco[i-1],varsTd[i-1],sol1,epsor,
Sequence@@FilterRules[opt,Options[phiStep]]];
If[Bco[i]===$Failed,Throw[Return[$Failed]]];
,{i,1,sz}];
,{i,sz}];
,
...
phi1=Table[Bco[i],{i,sz}];
If[print,Print[ToString[SetAccuracy[SessionTime[]-time,3]]," s"]];
allVars=Table[varsTd[i],{i,1,sz}];
{phi1,allVars}];
...
equStep[psi1_,phi1In_,epsor_,varsTdIn_,degsIn_,OptionsPattern[]]:=Module[{graphvars,sz,varsTd,LaurentGraph,equGraph,
...
print=!OptionValue["Silent"];
graphvars=OptionValue["Variables"];
If[graphvars===Automatic,graphvars=Variables[phi1In]];
If[Length[graphvars]<2,Message[nthO::badvars];Return[$Failed]];
eps=graphvars[[1]];
...
basisChange[amatrix_,Tmatrix_,OptionsPattern[]]:=Module[{x,graphvars,sz,Tgraph,Anode,TAnode,AddNode,outnode,opt,xv,learn,
...
phi2=psiCalc[bmatrix,Sequence@@FilterRules[opt,Options[psiCalc]]];
